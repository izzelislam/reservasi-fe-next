...
const NavnarSection = () => {
  ...
  return (
    <>
      {/* navbar menu */}
      <nav className="py-6 sticky top-0 bg-white z-50">
                  ...
                  endContent={
                    ...
                    >
                      {isVisible ? (
                        ...
                        <Icon icon="solar:eye-closed-bold-duotone" className="text-2xl text-default-400 pointer-events-none flex-shrink-0 cursor-pointer"/>
                      ) : (
                        <Icon icon="solar:eye-bold-duotone" className="text-2xl text-default-400 pointer-events-none flex-shrink-0 cursor-pointer"/>
      ...
      {/* mobile menu */}
      <div className={`bg-white fixed top-20 left-0 w-full h-full z-40 ${isOpenMenuMobile ? 'translate-x-0' : 'translate-x-full'} transition-all ease-out-in duration-300`}>
        <div className="flex flex-col gap-6 px-6 py-12">
          <Link href="/">
            <p className="text-burem-200 text-sm">Home</p>
          </Link>
          <Link href="/explore">
            <p className="text-burem-200 text-sm">Explore</p>
          </Link>
          <Link href="/cek-pesanan">
            <p className="text-burem-200 text-sm">Cek Pesanan Anda</p>
          </Link>

          {
            !isLogin && Object.keys(user).length === 0 ? (
              <div className='flex flex-col gap-3'>
                <Button isLoading={isLoadingRegister} color="primary" onPress={() => {
                ...
                <Button disabled={isLoadingLogin} variant='bordered' isLoading={isLoadingLogin} onPress={() => {
                  setIsOpenMenuMobile(false)
                  // handleLogin()
                  setIsOpenModal(true)
                }} color="primary" startContent={<Icon icon="solar:login-bold-duotone" />}>
                  Masuk
                </Button>
              </div>
            ) : (
