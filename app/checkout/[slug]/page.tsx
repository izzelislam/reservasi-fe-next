  ...
  return (
    <div>
      <section className="bg-gray-100">
        ...
        {/* mobile floating */}
        <div className="fixed md:hidden left-0 right-0 bottom-0 z-10 bg-white p-6 rounded-lg rounded-t-xl drop-shadow-[0_0px_22px_rgba(0,0,0,0.50)]">
          <div className="mb-2 pb-4 border-b border-gray-300">
            <p className="font-semibold text-lg text-burem-500">Total bayar</p>
          </div>
          <div className="pb-4 mb-1 border-b border-gray-300">
            <p className="text-gray-500 text-sm mb-1">{room?.name}</p>
            <p className="text-primary font-bold text-2xl">{NumberToIdr(room?.price * getDiff(new Date(start!), new Date(end!)))}</p>
          </div>
          <div>
            <p className="text-gray-700 text-sm mb-2">Pilih metode pembayaran</p>
            <Select
          ...
          <div>
            <Button onClick={() => handleCheckout()} isLoading={loadingCo} isDisabled={loadingCo} color="primary" className="mt-4" size='lg' fullWidth endContent={<Icon icon="solar:card-bold-duotone"/>}>Booking Sekarang</Button>
          </div>
        </div>
        {/* end mobile floating */}
